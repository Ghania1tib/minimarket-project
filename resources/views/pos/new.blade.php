                                ...
                                <div class="product-card {{ $product->stok == 0 ? 'out-of-stock' : '' }}"
                                    ...
                                    <div class="product-image mb-2 position-relative">
                                        @if($product->gambar && Storage::exists($product->gambar))
                                            <img src="{{ Storage::url($product->gambar) }}" alt="{{ $product->nama_produk }}" class="img-fluid">
                                        @else
                                    ...
                                    <!-- Badge Grosir -->
                                    @if($product->wholesale_rules && count($product->wholesale_rules) > 0)
                                        <div class="wholesale-badge">
                                            <i class="fas fa-tags me-1"></i>GROSIR
                                        </div>
                                        <div class="wholesale-rules">
                                            @foreach($product->wholesale_rules as $rule)
                                                ≥{{ $rule['min_quantity'] }}: {{ $rule['discount_percent'] }}%
                                            @endforeach
                                        </div>
                                    @endif
...
@push('scripts')
    <script>
        ...
        // PERBAIKAN: Fungsi update stok produk setelah transaksi berhasil
                ...
                if (productCard) {
                    ...
                    if (stockElement) {
                        ...
                        if (newStock === 0) {
                            ...
                            productCard.onclick = function() { void(0); };
                        }
                    }
                }
            });
        ...
        function showReceipt(data, paymentMethod) {
